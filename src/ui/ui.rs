...
static STONKS_PARAGRAPH: Lazy<Paragraph> =
    Lazy::new(|| Paragraph::new(STONKS_LINES.clone()).centered());
...
fn render_night(
    ...
    frame.render_widget(&*STONKS_PARAGRAPH, v_split[0]);
...
pub(crate) fn render_stonk(
    ...
    let x_labels: Vec<Span> = [min_x_bound, max_x_bound]
        .iter()
...
pub fn render(
    ...
    let ui_options = &agent.ui_options;
