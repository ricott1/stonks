...
pub struct Stonk {
    ...
    pub shareholders: Vec<(AgentId, u32)>, // List of shareholders, always sorted from biggest to smallest.
...
impl Stonk {
    ...
    fn allocate_shares(&mut self, amount: u32) -> AppResult<()> {
        if amount > self.available_amount() {
            return Err(anyhow!(
                "Amount is greater than number of available shares."
            ));
        }
    ...
    pub fn allocate_shares_to_agent(&mut self, agent_id: AgentId, amount: u32) -> AppResult<()> {
        self.allocate_shares(amount)?;

        if let Some((_, old_amount)) = self
            .shareholders
            .iter_mut()
            .find(|(holder, _)| *holder == agent_id)
        {
            *old_amount += amount
        } else {
            self.shareholders.push((agent_id, amount))
        }
    ...
    fn deallocate_shares(&mut self, amount: u32) -> AppResult<()> {
        if amount > self.allocated_shares {
            return Err(anyhow!(
                "Amount is greater than number of allocated shares."
            ));
        }
    ...
    pub fn deallocate_shares_to_agent(&mut self, agent_id: AgentId, amount: u32) -> AppResult<()> {
            ...
            .find(|(holder, _)| *holder == agent_id)
        {
            if amount > *old_amount {
                return Err(anyhow!(
                    "Amount is greater than number of shares owned by agent."
                ));
            }
            *old_amount -= amount
        } else {
            return Err(anyhow!("Agent is not a shareholder"));
        }
        self.sort_shareholders();
        info!("New shareholders: {:#?}", self.shareholders);
